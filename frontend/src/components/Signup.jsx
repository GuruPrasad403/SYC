...
export default function Signup(){
    ...
    const signup = async ()=>{
        const res = await createUserWithEmailAndPassword(auth,email,passwordlogin)

    }

    return(
        <>
        <div className="flex justify-center flex-wrap items-center w-screen h-screen sm:h-100vh bg-zinc-800">
    ...
    <div className="text-2xl  text-[#76797e] pb-8">SIGN UP</div>
    <form className="flex flex-col">
    <div className="pb-2">
            <label htmlFor="Name" className="block mb-2 text-sm font-medium text-[#ffffff]">Name</label>
    ...
    <circle cx="12" cy="7" r="4"></circle>
                ...
                <input type="Text" name="Name" id="Name" className="pl-12 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 sm:text-sm rounded-lg ring ring-transparent focus:ring-1 focus:outline-none focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4" placeholder="Enter Your Name" autoComplete="off" />
            </div>
        </div>
        ...
        <div className="pb-2">
            ...
            <div className="relative text-gray-400">
                ...
                className="pl-12 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 sm:text-sm rounded-lg ring ring-transparent focus:ring-1 focus:outline-none focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4" placeholder="example@gmail.com" autoComplete="off" />
            </div>
        </div>
        <div className="pb-6">
            ...
            <div className="relative text-gray-400" ref={password}>
                ...
                <input type={type} ref={type} name="password" id="password" onChange={(e)=>{
                    setpasswordlogin(e.target.value)
                }}
                placeholder="••••••••••" className="pl-12 mb-2 bg-gray-50 text-gray-600 border focus:border-transparent border-gray-300 sm:text-sm rounded-lg ring ring-transparent focus:ring-1 focus:outline-none focus:ring-gray-400 block w-full p-2.5 rounded-l-lg py-3 px-4" autoComplete="new-password" aria-autocomplete="list" />
            </div>
        </div>
        <button type="submit" onClick={signup} className=" hover:bg-zinc-600 hover:text-balck w-full text-[#ffffff] bg-[#232327] focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-6">signup</button>
        <div className="text-sm font-light text-[#a6a8ac]">Existing User? <a onClick={()=> navigator("/")} className="font-medium text-[#ffffff] hover:underline hover:cursor-pointer">Sign In</a></div>
    </form>
